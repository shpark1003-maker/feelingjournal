    ...
    if (fullGalleryGrid) {
        if (filteredFragments.length === 0) {
            ...
            fullGalleryGrid.innerHTML = filteredFragments.map(f => {
                const isSelected = notebookState.selectedPageIds.has(f.id);
                const isE2e = !!f.isE2e;
                return `
                <div class="photo-card full-memory-item cursor-pointer relative ${isSelected ? 'ring-2 ring-primary' : ''}" data-id="${f.id}">
                    <div class="aspect-square overflow-hidden rounded-md bg-surface-container relative">
                        ...
                        ${notebookState.selectModeActive ? `
                        <!-- Checkbox overlay -->
            ...
            fullGalleryGrid.querySelectorAll('.full-memory-item').forEach(art => {
                art.addEventListener('click', (e) => {
                    ...
                    console.log(`--- [DEBUG SELECTION] Clicked card id:`, id, `notebookState.selectModeActive:`, notebookState.selectModeActive);
                    ...
                    if (notebookState.selectModeActive) {
                        if (page.isE2e) {
                        ...
                        if (notebookState.selectedPageIds.has(id)) {
                            notebookState.selectedPageIds.delete(id);
                        } else {
                            notebookState.selectedPageIds.add(id);
                        }
                        console.log(`--- [DEBUG SELECTION] Current selected ids:`, Array.from(notebookState.selectedPageIds));
                        ...
                        if (cb) cb.checked = notebookState.selectedPageIds.has(id);

                        // Update selected count
                        const countText = document.getElementById('v2-gallery-select-count');
                        if (countText) countText.textContent = `${notebookState.selectedPageIds.size}개 선택됨`;
