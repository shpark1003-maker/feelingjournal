...
export async function loadMessages() {
    try {
        ...
        if (container) {
            renderedMessageIds.clear(); // 돔 클리어 전 중복 세트 초기화
            container.innerHTML = '';
...
export async function callChatAI(msg) {
    ...
    if (container) {
        ...
        typingIndicator.className = 'message received ai-message typing-indicator';
        typingIndicator.style.alignSelf = 'flex-start';
        typingIndicator.innerHTML = `
            <div style="display: flex; align-items: flex-start; gap: 8px; width: 100%;">
                <div class="chat-msg-avatar-fallback" style="width: 34px; height: 34px; border-radius: 50%; background: #e8f0e0; display: flex; align-items: center; justify-content: center; font-size: 1.1rem; border: 2px solid #5d574d; margin-right: 8px; color: #5d574d; box-shadow: 1px 1px 0px rgba(0,0,0,0.05);">💭</div>
                <div style="flex: 1; display: flex; flex-direction: column; align-items: flex-start;">
                    <div class="message-content paper-note active-bubble" style="padding: 12px 16px; border-radius: 2px 12px 12px 12px; font-size: 14px; max-width: 95%; background: #fffef0; border: 2px solid #5d574d; color: #8b8273; box-shadow: 2px 2px 0px rgba(93,87,77,0.15); font-style: italic;">
                        비서가 답변을 작성 중입니다...
                    </div>
                </div>
            </div>
    ...
    try {
        ...
        if (data.success) {
            // 중복 요청 방지 락에 걸린 경우 조용히 넘어감
            if (data.duplicated) return;
            
            // 수동 렌더링 추가 (Realtime 지연 방지)
            if (data.answer) {
                // AI 응답의 임시 형태 (실제 DB 삽입과 별개로 UI에 먼저 띄움)
                const mockAiMsg = {
                    id: 'temp-' + Date.now(),
                    content: data.answer,
                    created_at: new Date().toISOString(),
                    user_email: title.includes('영희') || title.includes('철수') || title.includes('민수') || title.includes('데모') 
                        ? `friend-${encodeURIComponent(title.replace('💬 ', '').replace('님과의 대화', '').trim())}@feeling.journal` 
                        : 'ai@feeling.journal',
                    sender_id: store.currentUser?.id
                };
                appendMessage(mockAiMsg);
            }
        } else {
...
export function setupChatUI() {
    ...
    sendBtn?.addEventListener('click', async () => {
        ...
        isSending = true;
        input.value = ''; // Prevent double send
        ...
        try {
            ...
            if (title.includes('와 대화') || title.includes('비서')) {
                // AI 대화방이면 멘션 없이 항상 응답
                callChatAI(content);
            } else if (friend) {
...
export async function checkFriendSos() {
    ...
    if (list) {
        ...
        const aiFriendHtml = `
        <div class="page-item friend-item ai-friend ${isAiActive ? 'active' : ''}" data-action="open-chat-ai" onclick="void(0)" style="display:flex; align-items:center; gap:12px;">
            ${aiAvatarHtml}
        ...
        let htmlContent = aiFriendHtml + friends.map(f => {
            ...
            return `
            <div class="page-item friend-item ${isSos ? 'sos' : ''} ${store.currentRoomId === f.id ? 'active' : ''}" data-action="open-chat-friend" data-id="${f.id}" data-nickname="${f.nickname}" onclick="void(0)" style="display:flex; align-items:center; gap:12px;">
    ...
    if (chatRoomList) {
        ...
        let roomsHtml = `
        <div data-action="open-chat-ai" onclick="void(0)" class="bg-white/40 backdrop-blur-sm p-4 rounded-xl flex items-center gap-4 hover:bg-white/60 cursor-pointer transition-all border ${isAiActive ? 'border-primary bg-white/70' : 'border-transparent'} hover:border-primary-container/30">
        ...
        roomsHtml += friends.map(f => {
            ...
            return `
            <div data-action="open-chat-friend" data-id="${f.id}" data-nickname="${f.nickname}" onclick="void(0)" class="bg-white/40 backdrop-blur-sm p-4 rounded-xl flex items-center gap-4 hover:bg-white/60 cursor-pointer transition-all border ${isRoomActive ? 'border-primary bg-white/70' : 'border-transparent'} hover:border-primary-container/30">
