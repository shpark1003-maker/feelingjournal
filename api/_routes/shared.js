    ...
    try {
        ...
        if (listRes.ok) {
            ...
            if (listData.items && listData.items.length > 0) {
                // Filter calendars: selected, not hidden, role in owner/writer/reader
                calendars = listData.items.filter(cal => 
                    cal.selected === true && 
                    cal.hidden !== true && 
                    ['owner', 'writer', 'reader'].includes(cal.accessRole)
                );
                // If filtering results in empty, fallback to primary
        ...
        // Deduplication keys: 1) iCalUID, 2) calendarId:eventId, 3) normalized (summary + start + end)
