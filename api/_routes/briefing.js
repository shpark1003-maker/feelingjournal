...
3. **뉴스 요약 및 메모화 지침 (중요)**: 
   ...
   - 브리핑 본문 중간에 반드시 **'[오늘의 뉴스]'**를 소제목으로 잡고, 그 하위에 각 뉴스 카테고리(정치, 경제 등)를 부제목으로 표시한 후, 섹션별로 그날의 주요 뉴스를 딱 3개만 한 줄 요약 리스트 형식으로 출력하십시오.
   - 예시 포맷:
     [오늘의 뉴스]
     • 경제:
       - 1. 코스피 8000포인트 돌파, 전일 대비 1.5% 상승
       - 2. 정부 반도체 클러스터 지원안 발표
       - 3. 원달러 환율 하락 안정세 유지
     • 정치:
       - 1. 국회 법안 상정 및 심사 본격화
       - 2. 청와대 신임 수석 발표
...
9. 뉴스정리는 반드시 '[오늘의 뉴스]'라는 소제목 하위에 카테고리/섹션별(정치, 경제 등) 부제목을 노출하고, 그 하위에 기사 제목 한 줄 요약 리스트만 최대 3개씩 나열해야 합니다. 절대 설명조 문장으로 길게 쓰지 말고 딱 한 줄로만 간결하게 메모 형식으로 요약하십시오.
`;
